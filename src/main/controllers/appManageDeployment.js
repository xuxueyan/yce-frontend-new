define(function (){
    ...
    return function appManageDeploymentCtrl($scope, $http, $rootScope, atomicNotifyService, $timeout){
        ...
        var applyInitToken = yce.api.apply.applyInit($http, data);
        utils.responseHandler(applyInitToken, function(data){
            if(data.code == 0){
        ...
        $scope.applyLabelArray = [];

        $scope.addApplyLabel = function (){
            $scope.applyLabelArray.push({});

        };

        $scope.deleteApplyLabel = function (index){
            $scope.applyLabelArray.splice(index, 1);
        };
        ...
        $scope.$watch('applyData.appName', function(newValue){
            if(newValue){
                ...
                timeout = $timeout(function(){
                    ...
                    utils.responseHandler(yce.api.apply.nameExist($http, data), function(data){
                        if(data.code == 1415){
                            $scope.applyNameExist = true;
                        }else{
                            $scope.applyNameExist = false;
                        }
                    });
        ...
        $scope.applySubmit = function (){
            ...
            //标签组 key&value
            if($scope.applyLabelArray.length >0){
                angular.forEach($scope.applyLabelArray, function (data, index){
